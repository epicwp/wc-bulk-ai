...
class Verbose_Logger implements Process_Logger {
    ...
    public function log_ai_response( CreateResponse $response ): void {
        ...
        if ( ! empty( $choice->message->toolCalls ) ) {
            \WP_CLI::log(
                \WP_CLI::colorize(
                    '%y🔧 Agent wants to call ' . \count( $choice->message->toolCalls ) . ' tool(s):%n',
                ),
            );
        }
        ...
        if ( $response->usage ) {
            \WP_CLI::log(
                \WP_CLI::colorize(
                    "%m📊 Tokens used: {$response->usage->totalTokens} (prompt: {$response->usage->promptTokens}, completion: {$response->usage->completionTokens})%n",
                ),
            );
        }
    ...
    public function log_max_iterations_reached(): void {
        \WP_CLI::log(
            \WP_CLI::colorize( '%r⚠️  Maximum iterations reached - task may not be complete%n' ),
        );
        \WP_CLI::log( '' );
    }
